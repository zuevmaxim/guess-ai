...
test("compareGuess - exact match should return correct index", async () => {
    ...
    assert.strictEqual(result.matchIndex, 0, "Exact match 'Pizza' should return index 0");
    assert.ok(result.similarity > 0.8, "Similarity should be above threshold");
});
...
test("compareGuess - similar match should return correct index", async () => {
    ...
    const result = await compareGuess(guess, answers);
    assert.strictEqual(result.matchIndex, 0, "Similar match 'New York City' should match 'New York' at index 0");
    assert.ok(result.similarity > 0.8, "Similarity should be above threshold");
});
...
test("compareGuess - case insensitive matching", async () => {
    ...
    const guess = "apple";
    const result = await compareGuess(guess, answers);
    assert.strictEqual(result.matchIndex, 0, "Case insensitive 'apple' should match 'Apple' at index 0");
    assert.ok(result.similarity > 0.8, "Similarity should be above threshold");
});
...
test("compareGuess - close synonym may not match due to threshold", async () => {
    ...
    assert.ok(result.matchIndex === null || result.matchIndex === 0, "Synonym 'Joyful' may or may not match 'Happy' depending on embedding similarity");
    assert.ok(typeof result.similarity === 'number', "Should always return a similarity value");
});
...
test("compareGuess - no match below threshold should return null", async () => {
    ...
    const result = await compareGuess(guess, answers);
    assert.strictEqual(result.matchIndex, null, "Unrelated guess 'Automobile' should return null");
    assert.ok(result.similarity < 0.8, "Similarity should be below threshold");
});
...
test("compareGuess - match answer at different positions", async () => {
    ...
    const guess = "Green";
    const result = await compareGuess(guess, answers);
    assert.strictEqual(result.matchIndex, 2, "Exact match 'Green' should return index 2");
    assert.ok(result.similarity > 0.8, "Similarity should be above threshold");
});
...
test("compareGuess - partial word in phrase", async () => {
    ...
    assert.strictEqual(result.matchIndex, 0, "Similar phrase 'Go to beach' should match 'Go to the beach' at index 0");
    assert.ok(result.similarity > 0.8, "Similarity should be above threshold");
});
...
test("compareGuess - very similar words", async () => {
    ...
    const guess = "Run";
    const result = await compareGuess(guess, answers);
    // Very similar root word may or may not match depending on embedding similarity
    assert.ok(result.matchIndex === null || result.matchIndex === 0, "Root word 'Run' may or may not match 'Running' depending on embedding similarity");
    assert.ok(typeof result.similarity === 'number', "Should always return a similarity value");
});
