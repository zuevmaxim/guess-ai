...
function mustArray(jsonText, expectedLength) {
  try {
    const data = JSON.parse(jsonText);
    if (Array.isArray(data) && data.length >= expectedLength && data.every((x2) => typeof x2 === "string")) {
  ...
  if (match) {
    try {
      ...
      if (Array.isArray(arr)) {
        return arr.filter((x2) => typeof x2 === "string").slice(0, expectedLength);
      }
    } catch {
    }
  }
...
async function compareGuess(guess, answers) {
  const inputs = [guess, ...answers];
  const emb = await openai.embeddings.create({
    model: "text-embedding-3-large",
    input: inputs
  });
  const vectors = emb.data.map((d2) => d2.embedding);
  const guessVec = vectors[0];
  let bestIdx = -1;
  let best = -1;
  for (let i2 = 0; i2 < answers.length; i2++) {
    const sim = cosineSim(guessVec, vectors[i2 + 1]);
    if (sim > best) {
      best = sim;
      bestIdx = i2;
    }
...
var Game = class _Game {
  ...
  static async create(topic, players) {
    const game2 = new _Game(players);
    const qs = await generateQuestions(topic);
    const questions = [];
    for (const q2 of qs) {
      const answers = await generateAnswers(q2);
      questions.push({ text: q2, answers, revealed: new Array(answers.length).fill(false) });
    }
    game2.questions = questions;
    game2.current = 0;
    game2.turn = 0;
    return game2;
  }
  ...
  allRevealed(qIndex) {
    const q2 = this.questions[qIndex];
    return q2 ? q2.revealed.every(Boolean) : true;
  }
  async handleGuess(rawGuess) {
    ...
    const q2 = this.currentQuestion;
    if (!q2) return { correct: false };
    const remainingAnswers = q2.answers.map((a2, i2) => ({ a: a2, i: i2 })).filter(({ i: i2 }) => !q2.revealed[i2]);
    const candidateAnswers = remainingAnswers.map((x2) => x2.a);
    ...
    if (matchIndexInRemaining !== null) {
      const revealedIndex = remainingAnswers[matchIndexInRemaining].i;
      q2.revealed[revealedIndex] = true;
      const points = this.pointsForRank(revealedIndex);
      this.players[this.turn].score += points;
      result = { correct: true, points, answer: q2.answers[revealedIndex] };
    }
    this.advanceTurn();
...
var server = import_http.default.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/") {
      const html = await (0, import_promises.readFile)(new import_url.URL("./index.html", import_meta.url));
    ...
    if (req.method === "GET" && url.pathname === "/state") {
      ...
      const state = {
        question: q2 ? q2.text : null,
        answers: q2 ? q2.answers : [],
        revealed: q2 ? q2.revealed : [],
